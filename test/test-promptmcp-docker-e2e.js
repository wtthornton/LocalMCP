...
/**
 * PromptMCP Docker E2E Test - Enhance Tool Only
 * 
 * Tests the promptmcp.enhance tool by calling the Docker MCP server directly
 * This is a proper E2E test that tests the actual user experience
 */
...
class PromptMCPDockerE2ETester {
  constructor() {
    this.results = [];
    this.scorecard = {
      overall: {
  ...
  /**
   * Call the MCP server enhance tool
   */
  async callEnhanceTool(prompt, context) {
    return new Promise((resolve, reject) => {
      ...
      process.on('close', (code) => {
        ...
        try {
          ...
          console.log('Debug: Looking for JSON in output...');
          ...
          for (let i = 0; i < lines.length; i++) {
            ...
            if (line.startsWith('{')) {
              console.log('Debug: Found potential JSON at line', i + 1, ':', line.substring(0, 100) + '...');
              if (line.includes('"result"') || line.includes('"error"') || line.includes('"jsonrpc"')) {
                try {
          ...
          if (!jsonResponse) {
            // If no JSON found, check if there's an error message in the output
            if (output.includes('Context7 integration not initialized')) {
              resolve({
                success: false,
                enhanced_prompt: prompt, // Return original prompt
                context_used: { repo_facts: [], code_snippets: [], context7_docs: [] },
                error: 'Context7 integration not available'
              });
              return;
            }
          ...
          if (jsonResponse.error) {
            // Handle expected error - Context7 not initialized
            if (jsonResponse.error.message.includes('Context7 integration not initialized')) {
  ...
  async testPrompt(testCase) {
    ...
    try {
      const result = await this.callEnhanceTool(testCase.prompt, testCase.context);
      ...
      const evaluation = this.scoreEnhancement(testCase, enhancedPrompt, contextUsed, responseTime, result.error);
   ...
   */
  async runAllTests() {
    console.log('🚀 Starting PromptMCP Docker E2E Evaluation - Enhance Tool Only');
    console.log('============================================================');
  ...
  generateScorecard() {
    ...
    const filename = `promptmcp-docker-e2e-evaluation-${timestamp}.json`;
...
// Main execution
async function main() {
  const tester = new PromptMCPDockerE2ETester();
