...
/**
 * PromptMCP E2E Test - Enhance Tool Only
 * 
 * Tests only the promptmcp.enhance tool functionality by calling the MCP server directly
...
class PromptMCPE2ETester {
  ...
  async testPrompt(testCase) {
    ...
    try {
      ...
      console.log(`✅ Test passed: ${evaluation.overallScore}/100`);
   ...
   */
  async runAllTests() {
    console.log('🚀 Starting PromptMCP E2E Evaluation - Enhance Tool Only');
    console.log('============================================================');
  ...
  generateScorecard() {
    ...
    const filename = `promptmcp-e2e-evaluation-${timestamp}.json`;
...
// Main execution
async function main() {
  const tester = new PromptMCPE2ETester();
